...
import static edu.gla.kail.ad.agents.DialogflowAgentAuthorizationSingleton.getProjectIdAndSessionsClient;

/**
 * This class is initialized once per session per agent. (Different sessions
 * have different instances for the same agent.) The request sent to the agent
 * is validated. (There are no invalid characters which can make dialogflow
 * throw errors.) // TODO(Adam) Use shutdown() method for closing _sessionClient
 * stream? Implement it in AgentInterface?
 */
public class DialogflowAgent implements AgentInterface {
         ...
         * @throws IOException - T setUpAgent method may throw exception if the data
         *                     passed in the tupleOfProjectIdAndAuthorizationFile is
         *                     invalid.
         */
        public DialogflowAgent(String sessionId, AgentConfig agent) throws IOException {
         ...
         *
         * @throws IOException - When a there is something wrong with getting ProjectID
         *                     and Session Client of the Agent.
         */
        private void setUpAgent(AgentConfig agent) throws IOException {
                Tuple<String, SessionsClient> projectIdAndSessionsClient = getProjectIdAndSessionsClient(agent);
        ...
        /**
         * Validate the inputInteraction for Dialogflow usage. Checks: inputInteraction,
         * type, respective type field, language code.
         *
         * @param inputInteraction - A data structure (implemented in log.proto) holding
         *                         the incoming interaction that is being sent to an
         *                         agent.
         * @throws IllegalArgumentException - Thrown when the input is invalid.
         */
        private void validateInputInteraction(InputInteraction inputInteraction) throws IllegalArgumentException {
                checkNotNull(inputInteraction, "The passed inputInteraction is null!");
                if (checkNotNull(inputInteraction.getLanguageCode(),
                                "The inputInteraction LanguageCode " + "is set to null!").isEmpty()) {
                ...
                if (checkNotNull(inputInteraction.getType(), "The inputInteraction type is null!").toString()
                                .isEmpty()) {
                        throw new IllegalArgumentException("The inputInteraction type is not set!");
                }
                switch (inputInteraction.getType()) {
                case TEXT:
                        if (checkNotNull(inputInteraction.getText(),
                                        String.format(ERROR_MESSAGE, "TEXT", "a null", "text")).isEmpty()) {
                                throw new IllegalArgumentException(
                                                String.format(ERROR_MESSAGE, "TEXT", "an empty", "text"));
                        }
                        break;
                case ACTION:
                        if (checkNotNull(inputInteraction.getActionTypeList(),
                                        String.format(ERROR_MESSAGE, "ACTION", "a null", "action")).isEmpty()) {
                                throw new IllegalArgumentException(
                                                String.format(ERROR_MESSAGE, "ACTION", "an empty", "action"));
                        }
                        break;
                case AUDIO:
                        if (checkNotNull(inputInteraction.getAudioBytes(),
                                        String.format(ERROR_MESSAGE, "AUDIO", "a null", "audio")).isEmpty()) {
                                throw new IllegalArgumentException(
                                                String.format(ERROR_MESSAGE, "AUDIO", "an empty", "audio"));
                        }
                        break;
                default:
        ...
        /**
         * Return Query input for any type of inputInteraction the user may get: audio,
         * text or action.
         *
         * @param inputInteraction - A data structure (implemented in log.proto) holding
         *                         the incoming interaction that is being sent to an
         *                         agent.
         * @return queryInput - A data structure which holds the query that needs to be
         *         send to Dialogflow.
         */
        private DetectIntentResponse detectIntentResponseMethod(InputInteraction inputInteraction) {
                ...
                // (inputInteraction type).
                switch (inputInteraction.getType()) {
                case TEXT:
                        TextInput.Builder textInput = TextInput.newBuilder().setText(inputInteraction.getText())
                                        .setLanguageCode(inputInteraction.getLanguageCode());
                        return _sessionsClient.detectIntent(_session,
                ...
                case AUDIO:
                        ...
                        return _sessionsClient.detectIntent(DetectIntentRequest.newBuilder()
                                        ...
                                        .setInputAudio(ByteString.copyFrom(inputAudio)).build());
                case ACTION:
                        ...
                        // (eventInput).build());
                        throw new IllegalArgumentException(
                                        "The ACTION method for DialogFlow is not" + " yet supported" + "."); // TODO(Adam):
                                                                                                             // implement;
                default:
                        throw new IllegalArgumentException("Unrecognised interaction type.");
                }
         ...
         *
         * @throws IllegalArgumentException - The exception is being thrown when the
         *                                  type of the interaction requested is not
         *                                  recognised or supported.
         */
        @Override
        public ResponseLog getResponseFromAgent(InteractionRequest interactionRequest) throws IllegalArgumentException {
                DetectIntentResponse response = detectIntentResponseMethod(interactionRequest.getInteraction());
                ...
                ResponseLog.Builder responseLogBuilder = ResponseLog.newBuilder()
                                .setResponseId(response.getResponseId()).setTime(timestamp)
                                .setServiceProvider(ServiceProvider.DIALOGFLOW).setRawResponse(response.toString());

                SystemAct.Builder systemActBuilder = SystemAct.newBuilder().setAction(queryResult.getAction())
                                .setInteraction(OutputInteraction.newBuilder().setType(InteractionType.TEXT) // TODO(Adam):
                                                                                                             // If more
                                                                                                             // advanced
                                                                                                             // Dialogflow
                                                // agents can send a response with different interaction type, this
                                                // needs
                                                // to be changed.
                                                .setText(queryResult.getFulfillmentText()).build());

                for (Context context : queryResult.getOutputContextsList()) {
                        // Set the slot's name and value for every Slot.
                        for (Map.Entry<String, Value> parameterEntry : context.getParameters().getFieldsMap()
                                        .entrySet()) {
                                systemActBuilder.addSlot(Slot.newBuilder().setName(parameterEntry.getKey())
                                                .setValue(parameterEntry.getValue().toString()).build());
                        }
                }
