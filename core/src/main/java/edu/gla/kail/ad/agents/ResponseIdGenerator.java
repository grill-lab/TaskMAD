...
/**
 * Generate a response identifier. This is based on Firestore's Push ID
 * generator. It is from: https://gist.github.com/jfbyers/d142503b2e41556b5684
 */
public class ResponseIdGenerator {
    ...
    // the last
    // characters we generated because in the event of a collision, we'll use those
    // same
     ...
     *
     * 1. They're based on timestamp so that they sort *after* any existing ids. 2.
     * They contain 72-bits of random data after the timestamp so that IDs won't
     * collide with other clients' IDs. 3. They sort *lexicographically* (so the
     * timestamp is converted to characters that will sort properly). 4. They're
     * monotonically increasing. Even if you generate more than one in the same
     * timestamp, the latter ones will sort after the former ones. We do this by
     * using the previous random bits but "incrementing" them by 1 (only in the case
     * of a timestamp collision).
     *
     * @return
     */
     ...
     * Consider a response identifier that includes time + randomness, similar to
     * Firestore: 48 bits of timestamp + 72 bits of randomness; see
     * https://gist.github.com/mikelehen/3596a30bd69384624c11. TODO(Jeff): Factor ID
     * generation out into a library for generating identifiers.
     * 
